  ...
  const projectCards = [
    {
      ...
      details: "Usage of electric 3W cargo in Telangana for logistics",
    },
    {
  ...
  return (
    <div>
      ...
      <div className="flex justify-between gap-4">
        <div className="w-1/2 grid grid-cols-2 gap-4">
          {projectCards.map((card, i) => (
            ...
            >
              <p className="font-bold text-slate-600">{card.title}</p>
              <p className=" text-slate-500 ">{card.details}</p>
            </div>
          ))}
        </div>
        <div className="w-1/2">
          <img src={solar_pump} className="w-full rounded-md" alt="" />
        </div>
      </div>
      <br />
      ...
      <div className="px-3 py-2 flex justify-between items-center bg-slate-700 text-slate-50 text-[18px] rounded-md">
        <p>
          Total serialized carbon credit for this project that have been
          registred.
        </p>
